...
if(isset($_POST['name']) and isset($_POST['mail']) and isset($_POST['messageForm'])){
    ...
    if($name == '') {
        echo json_encode(array('info' => 'error', 'msg' => "por favor ingrese un nombre"));
        exit();
    } else if($fecha == ''){
        echo json_encode(array('info' => 'error', 'msg' => "por favor ingrese una fecha"));
        exit();
    }else if$fecha1 == ''{
        echo json_encode(array('info' => 'error', 'msg' => "por favor ingrese un fecha"));
        exit();
    } else if($mail == '' or check_email($mail) == false){
        echo json_encode(array('info' => 'error', 'msg' => "Please enter valid e-mail."));
        exit();
    }  else if($messageForm == ''){
        echo json_encode(array('info' => 'error', 'msg' => "por favor ingrese una descripcion."));
        exit();
    } else {
        $to = __TO__;
